module.exports.config = {
	...
	description: "Quản lý folders/files",
			...
			case "view":
				{
					...
					if (/\.js$/.test(p))
						copyFileSync(p, (t = p.replace(".js", ".txt")));
					api.sendMessage(
						{
			...
			case "create":
				{
					...
					fs[
						["mkdirSync", "writeFileSync"][
							(t = /\/$/.test(event.args[1]) ? 0 : 1)
						]
					](
						$.directory + event.args[1],
						[, event.args.slice(2).join(" ")][t]
					);
					api.sendMessage(
						`✅ Đã tạo ${["folder", "file"][t]} path: ${event.args[1]}`,
						event.threadID,
						event.messageID
					);
				}
			...
			case "zip":
				catbox(
					zip(
						$.data
							.filter((e, i) =>
								event.args.slice(1).includes(String(i + 1))
							)
							.map((e) => e.dest)
					)
				).then((link) =>
...
function openFolder(a, b, c) {
	let folders_files = readdirSync(c)
		.reduce(
			(o, e) => (o[statSync(c + "/" + e).isFile() ? 1 : 0].push(e), o),
			[[], []]
		)
...
function size_folder(folder = "") {
	...
	if (folder === _node_modules_path) {
		...
		if (_node_modules.length !== _node_modules_.length)
			(_node_modules = _node_modules_), (_node_modules_bytes = undefined);
