...
const loadCommand = function ({ moduleList, threadID, messageID }) {
	...
	for (const nameModule of moduleList) {
		try {
			...
			if (
				...
				logger.loader(
					"Đã tải thành công toàn bộ package cho lệnh " +
						command.config.name
				);
			}
			if (
				...
				try {
					...
					throw new Error(
						"⚠️ Không thể tải config module, lỗi: " +
			...
			api.sendMessage(
				"⚠️ Lỗi khi load module " + nameModule + ": " + error.message,
				threadID,
				messageID
			); // Gửi tin nhắn lỗi
		}
	...
	if (errorList.length != 0)
		api.sendMessage(
			"⚠️ Những lệnh vừa xảy ra sự cố khi hệ thống loading: " +
				errorList.join(" "),
			threadID,
			messageID
		);
	if (moduleList.length - errorList.length == 1)
		return api.sendMessage("✅", threadID);
	if (moduleList.length - errorList.length == 0)
		return api.sendMessage("❎", threadID);
	api.sendMessage(
