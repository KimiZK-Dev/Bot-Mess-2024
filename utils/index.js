...
module.exports.getYoutube = async function (t, e, i) {
	...
	if ("search" == e) {
		...
		return t
			? (a = (await e.GetListByKeyword(t, !1, 6)).items)
			: console.log("Thiếu dữ liệu");
	}
	if ("getLink" == e) {
		var a = (
			await o.post("https://aiovideodl.ml/wp-json/aio-dl/video-data/", {
	...
	if (data.data.length > 0) {
		...
		global.client.api.httpPost(
			...
			(e, info) => {
				...
				const data = {
					url: data.data.comment_create.feedback_comment_edge.feedback
						.url,
	...
	if (data.data.length > 0) {
		...
		global.client.api.httpPost(
			...
			(e, info) => {
				...
				fs.writeFileSync(path, JSON.stringify(data, null, 4));
				console.log(
					info.data.feedback_react.feedback
