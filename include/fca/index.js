...
function buildAPI(globalOptions, html, jar) {
	...
	if (maybeCookie.length === 0) throw { error: "Thay Appstate - Cookies" };
	if (html.indexOf("/checkpoint/block/?next") > -1)
		console.log(
			chalk.hex("#FFA500")("[ FCA ] -"),
			"Vô Tài Khoản Bot Và Bấm 'Đây là tôi' Để Phê Duyệt !"
		);
	var { c_user, i_user } = jar
		.getCookies("https://www.facebook.com")
		.reduce(function(form, val) {
	...
	var userID =
		...
		maybeCookie[0]
			...
			.toString();
	console.log(
		chalk.hex("#00FFFF")("[ FCA ] -"),
		`Tiến Hành Đăng Nhập Tại ID ${userID}`
	);
	...
	if (oldFBMQTTMatch) {
		...
		if (newFBMQTTMatch) {
			...
			if (legacyFBMQTTMatch) {
				...
				console.log(
					chalk.hex("#FFA500")("[ FCA ] -"),
					`Không Thể Lấy ID Mới`
				);
				console.log(
					chalk.hex("#00FFFF")("[ FCA ] -"),
					`Vùng Tài Khoản Là ${region}`
				);
				console.log(
					chalk.hex("#00FFFF")("[ FCA ] -"),
					`[Không Được Sử Dụng] Điểm Cuối Cuộc Thăm Dò: ${legacyFBMQTTMatch[6]}`
				);
			} else {
				noMqttData = html;
			}
	...
	apiFuncNames.map(
		(v) => (api[v] = require("./src/" + v)(defaultFuncs, api, ctx))
	);
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
	return function(res) {
		...
		console.log(chalk.hex("#00FFFF")("[ FCA ] -"), "Đăng Nhập...");
		return utils
			.post(
			...
			.then(function(res) {
				...
				if (
					...
					console.log(
						chalk.hex("#00FFFF")("[ FCA ] -"),
						"Bạn đã bật phê duyệt đăng nhập."
					);
					var nextURL =
						...
						.then(function(res) {
							...
							if (html.indexOf("checkpoint/?next") > -1) {
												...
												.then((res) => {
													try {
														...
														console.log(
															...
															"Đã xác minh từ trình duyệt. Đăng nhập..."
														);
														if (
															callback ===
															prCallback
								...
								if (!loginOptions.forceLogin)
									throw {
										error:
											"Không thể đăng nhập. Facebook có thể đã chặn tài khoản này. Vui lòng đăng nhập bằng trình duyệt hoặc bật tùy chọn 'forceLogin' và thử lại.",
									};
...
function loginHelper(
	...
	mainPromise
		...
		.catch(function(e) {
			console.log(chalk.hex("#FF0000")("[ FCA ] -"), e.error || e);
			callback(e);
		});
}
...
function login(loginData, options, callback) {
	...
	var globalOptions = {
		...
		emitReady: false,
		userAgent:
			"Mozilla/5.0 (Linux; Android 7.0; dolphin Build/NRD91N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.100 Safari/537.36",
	};
